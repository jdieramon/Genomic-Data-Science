...
## ------------------------------------------------------------------------
edata = edata[rowMeans(edata) > 5000,] #require the mean > some value
dim(edata)
edata = log2(edata + 1) #transform

# By default calculates the distance between rows
dist1 = dist(t(edata)) #I want d between samples: transpos
...
# '''Define clusters: color_labels()
dend = color_labels(hclust1,4,col=1:4) # define 4 clusters
plot(dend)
dend = color_labels(hclust1,3,col=1:3) # define 3 clusters
plot(dend)

## ------------------------------------------------------------------------
# '''Define clusters: labels_colors()
labels_colors(dend) = c(rep(1,10),rep(2,9)) #define labels 10 times label1 + 9 times label2
plot(dend)
...
## ------------------------------------------------------------------------
# '''How many genes belong to each cluster?
